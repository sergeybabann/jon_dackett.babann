<?php
// Данные для урока
$pageTitle = 'Глава 7: Обработка ошибок и исключений';
$lessonNumber = 7;
$lessonTitle = 'Обработка ошибок и исключений';

// Навигация
$prevLesson = '../06_chapter/06_chapter.php';
$nextLesson = '../08_chapter/08_chapter.php';

// Содержимое урока
$lessonContent = '
    <div class="container">
    <ul>
    <li>Описание</li>
    </ul>
    </div>
    
    <h2>Практическое  выполнение кода</h2>
    <a href="02_data-type-practice.php" target="_blank">🔗 Типы данных, циклы, массивы →</a>
    <a href="02_loops-practice.php" target="_blank">🔗 Циклы →</a>
';

include '../../templates/lesson_template.php';
?>